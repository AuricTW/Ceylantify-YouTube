(() => {
  const OVERLAY_COUNT = 26;
  const OVERLAY_CLASS = "louis-thumbnail-overlay";
  const HOST_CLASS = "louis-thumbnail-host";
  const PREVIEWING_CLASS = "louis-thumbnail-previewing";
  const APPLIED_ATTR = "data-louis-overlay-applied";
  const POSITION_ATTR = "data-louis-position-applied";
  const HOVER_LISTENERS_ATTR = "data-louis-hover-listeners";
  ...
  function scanThumbnails(root = document) {
    ...
    if (applied > 0) {
      console.info(`[Louis Overlay] applied ${applied} thumbnail overlays`);
    }
  }
